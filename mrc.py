...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 14 16:12:04 2014
...
class Mrc2:
    ...
    def initHdrForArr(self, arr, zAxisOrder=None,**kwargs):
        """
        ...
        init_simple(self.hdr, mrcmode, arr.shape,**kwargs)
        if arr.ndim == 2:
            pass
        elif arr.ndim == 3:
            if   zAxisOrder[0] == 'z':
...
def shape(fn):
    m     = Mrc2(fn)
    shape = tuple(m.hdr.Num[[2,1,0]]) #np.concatenate([np.array([len(files)]), m.hdr.Num[[1,0]]])
    m.close()    
    return shape     
...
def load(fn):
    im,psize = load_psize(fn)
    return im

def save_eman(V,fn,**kwargs):  
    import EMAN2
    psize = kwargs.pop('pixel_size',1.0)    
    VEM   = EMAN2.EMNumPy.numpy2em(V)
    VEM.set_attr('apix_x',psize)
    VEM.set_attr('apix_y',psize)    
    VEM.set_attr('apix_z',psize)    
    VEM.set_attr('origin_x',0.0)    
    VEM.set_attr('origin_y',0.0)    
    VEM.set_attr('origin_z',0.0)    
    VEM.write_image(fn)
    del VEM

def save(a, fn, ifExists='overwrite', zAxisOrder=None,
         hdr=None, hdrEval='',
         calcMMM=True,
         extInts=None, extFloats=None, **kwargs):             
    """
    ...
    pixel_size=x, pixel size in angstroms
...
#        else:
#            raise RuntimeError, "not overwriting existing file '%s'"%fn
    #print 'Inside old mrc'        
    m = Mrc2(fn, mode='w')
    m.initHdrForArr(a, zAxisOrder,**kwargs)
    if hdr is not None:
        initHdrArrayFrom(m.hdr, hdr)
        
    m.hdr.mmm1 = (a.min(),a.max(),a.mean())
...
def MrcMode2dtype(mode):
    ...
    if mode<0 or mode>7:
        raise ValueError, "Prism file supports pixeltype 0 to 7 (%d given)" % mode
...
def init_simple(hdr, mode, nxOrShape, ny=None, nz=None, **kwargs):
    """note: if  nxOrShape is tuple it is nz,ny,nx (note the order!!)
    """
    psize = kwargs.pop('pixel_size',1)
    if ny is nz is None:
        if len(nxOrShape) == 2:
    ...
    hdr.m   = (nx,ny,nz)  # CHECK : should be nx,ny,nz ??
    hdr.d   = (nx*psize,ny*psize,nz*psize)
...
#    3) convert(relabel) dtype=bool to dtype=uint8 (arr.view(N.uint8))
